...
#### 配置项
- `autoScanOnStartup`: 布尔值，控制启动时是否自动扫描媒体库
- `libraryPath`: 字符串，存储媒体库根目录路径

#### 会话防循环保护机制
- 使用 sessionStorage 存储 `medex:autoScanFired` 标记
- 避免同一会话内重复触发自动扫描
- 首次扫描后标记当前时间戳，后续扫描跳过

#### 事件驱动的扫描流程
1. **检查条件**：读取 localStorage 中的 autoScanOnStartup 和 libraryPath
2. **会话保护**：检查 sessionStorage 中的扫描标记
3. **触发扫描**：dispatch medex:scan-started 事件
4. **执行扫描**：invoke scan_and_index 命令
5. **完成通知**：dispatch medex:scan-completed 事件
6. **数据刷新**：dispatch medex:media-updated 事件
...
#### startScan 函数
- **参数**：path (string) - 媒体库目录路径
- **返回**：Promise<void> - 扫描完成后的 Promise
- **功能**：
  - 验证路径有效性
  - dispatch medex:scan-started 事件
  - 调用 scan_and_index 命令
  - dispatch medex:scan-completed 事件
  - 显示扫描完成提示
...
**章节来源**
- [src/pages/Settings.tsx:52-70](file://src/pages/Settings.tsx#L52-L70)