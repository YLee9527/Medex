...
- [src-tauri/Cargo.toml:1-25](file://src-tauri/Cargo.toml#L1-L25)
...
- Tauri 配置：前后端联动构建、窗口尺寸与可调整属性、窗口最小尺寸约束（minWidth: 1300, minHeight: 680）、安全协议作用域、打包目标与外部二进制、更新器配置。
...
- 窗口与安全：窗口尺寸与可调整属性；窗口最小尺寸约束（minWidth: 1300, minHeight: 680）增强用户体验和界面稳定性；安全协议启用资产作用域。
- 打包：开启打包、目标为 all、外部二进制包含 ffmpeg、启用更新器产物。
- 插件：更新器启用，配置检查端点与公钥。

**更新** 新增窗口最小尺寸约束，确保应用在小屏幕设备上仍能提供良好的用户体验

```mermaid
flowchart TD
Conf["tauri.conf.json"] --> Build["build.beforeDevCommand/beforeBuildCommand/devUrl"]
Conf --> App["app.windows/security<br/>minWidth/minHeight 约束"]
...
**图表来源**
- [src-tauri/Cargo.toml:13-25](file://src-tauri/Cargo.toml#L13-L25)
- [src-tauri/capabilities/default.json:1-15](file://src-tauri/capabilities/default.json#L1-L15)
- [src-tauri/build.rs:1-4](file://src-tauri/build.rs#L1-L4)
...
- 窗口约束：新增的最小尺寸约束（minWidth: 1300, minHeight: 680）确保界面元素在小窗口下仍能正常显示，提升用户体验稳定性。
...
- 窗口尺寸异常
  - 现象：应用窗口过小导致界面元素拥挤或布局错乱。
  - 处理：确认 minWidth 和 minHeight 约束设置合理，避免小于 1300x680 的窗口尺寸。
  - 参考：[src-tauri/tauri.conf.json:18-19](file://src-tauri/tauri.conf.json#L18-L19)
- 发布签名失败
  - 现象：CI 打包阶段签名参数缺失或错误。
  - 处理：检查仓库密钥与密码配置是否正确。
...
## 结论
Medex 的构建配置围绕"前端 Vite + TypeScript + TailwindCSS"与"后端 Tauri + Rust"的组合展开，强调开发端口一致性、类型安全前置、按需样式提取与多平台打包。通过 CI/CD 实现自动化发布，结合权限与能力声明保障功能可用性。新增的窗口最小尺寸约束（minWidth: 1300, minHeight: 680）显著提升了应用在小屏幕设备上的用户体验和界面稳定性。建议在后续迭代中持续优化内容扫描范围、细化权限清单与缓存策略，以进一步提升构建效率与安全性。
...
- 部署注意事项
  ...
  - 窗口最小尺寸约束（minWidth: 1300, minHeight: 680）确保应用在小屏幕设备上仍能提供良好的用户体验。