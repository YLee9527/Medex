...
function MediaCard({
  ...
  const resolvedVideoThumbnail = videoThumbnail
    ? toPreviewSrc(videoThumbnail)
    : ''
  ...
  const handleRemoveTag = async (tagName: string) => {
    ...
    try {
      const dbTags = await invoke<DbTag[]>('get_tags_by_media', {
        mediaId: mediaIdNum,
      })
      ...
      window.alert(`${t('mediaCard.removeTagFailedPrefix')}${String(error)}`)
    }
  }
    ...
    >
      ...
      {selected && (
        <div
          ...
          style={{ backgroundColor: theme.selectionOverlay }}
        />
      )}
        ...
        {mediaType === 'video' ? (
          resolvedVideoThumbnail ? (
            ...
            >
              ...
              />
              <span className="absolute">{t('mediaCard.generatingThumb')}</span>
            </div>
          )
        ) : showImage ? (
          <img
...
function areMediaCardPropsEqual(
  prev: Readonly<MediaCardProps>,
  next: Readonly<MediaCardProps>,
) {
  if (
    prev.id !== next.id ||
    prev.path !== next.path ||
